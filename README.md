...
**特徴:**
- ✅ **最新機能搭載**: アンケート紐づけ強化＋郵便番号ベース都道府県補完
- ✅ **メンテナンス性が高い**: 設定・ユーティリティ・メイン処理が分離
- ✅ **拡張しやすい**: 機能追加や修正が簡単

**ファイル構成:**
- `01_main_priority.py` - メイン処理
- `02_config.py` - 設定ファイル
- `03_data_utils.py` - ユーティリティ関数
...
**特徴:**
- ✅ **シンプル**: 1ファイルで完結
- ⚠️ **最新機能なし**: アンケート紐づけ強化と都道府県補完機能がない
- ⚠️ **メンテナンス性が低い**: すべてのコードが1ファイルにまとまっている

**推奨:** 新しい機能が必要な場合や、今後の機能拡張を考えている場合は、**新バージョン（01_main_priority.py）を使用してください。**
...
├── 01_main_priority.py     ← メインスクリプト（新バージョン）
├── 02_config.py            ← 設定ファイル
├── 03_data_utils.py        ← ユーティリティ関数
└── possim_customer_priority_maker.py  ← 旧バージョン（後方互換用）
```
